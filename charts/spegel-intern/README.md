...
| dnsPolicy | string | `""` | DNS policy for the Spegel pod. If empty, uses ClusterFirstWithHostNet when hostNetwork is true and ClusterFirst otherwise. |
| hostNetwork | bool | `false` | If true, runs the Spegel pod in the host network namespace. |
...
| spegel.containerdMirrorDialTimeout | string | `"2s"` | Dial timeout for generated containerd mirror host entries. |
...
| spegel.mirrorBlobTimeout | string | `"30m"` | Max duration spent fetching a blob from a mirror peer. Set to 0 to disable. |
| spegel.mirrorManifestTimeout | string | `"3s"` | Max duration spent fetching a manifest from a mirror peer. Set to 0 to disable. |
